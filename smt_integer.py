...
def _add_benders_feasibility_cut(
    ...
    neg_rhs_scaled = int(math.floor(neg_rhs_scaled_float + 1e-6))  # add small epsilon to handle numerical issues
    ...
    for j, (_, lits, weights) in enumerate(discrete_encodings):
        ...
        for lit, w in zip(lits, weights):
            ...
            if np.isclose(a_scaled, 0.0):
                continue
            weight_int = int(math.ceil(a_scaled - 1e-6))  # subtract small epsilon to handle numerical issues
            pos_lits.append(lit)
    ...
    while True:
        ...
        if status in (gp.GRB.OPTIMAL, gp.GRB.SUBOPTIMAL):
            ...
            cut_added, assumptions = _add_benders_optimality_cut(
                solver, vpool, discrete_encodings, model_rhs, discrete_obj_arr, A_d, lp, best_obj
            )
            if not cut_added:
                no_good = []
                for _, lits, _ in discrete_encodings:
...
def _add_benders_optimality_cut(
    ...
    bound_scaled = round(adjusted_rhs_float * scale) - 1
    ...
    try:
        _add_pb_constraint(solver, vpool, pos_lits, pos_weights, bound_scaled, '<')
